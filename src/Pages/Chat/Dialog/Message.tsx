...
const Message: React.FunctionComponent<{
  ...
  useEffect(() => {
    if (!isRead)
      store.subscribe(() => {
        ...
        const message = messages.filter((m: any) => m.message_id === props.id)[0]
  ...
  const generateMessageWithUrl = () => {
    ...
    urls.forEach(url => {
      ...
      if (split[0] !== '')
        res.push(<span>{split[0]}</span>)
      res.push(<a rel={'noopener'} href={url.includes('http') ? url : 'https://' + url}><span className={'chat_dialog__messages-message_top-bubble--link'}>{url}</span></a>)
      c = split[1]
    })
    return res
  }
