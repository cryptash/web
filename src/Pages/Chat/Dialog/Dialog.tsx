...
const Dialog: FunctionComponent<{
  ...
  const mes = useMemo(() => {
    ...
    props.messages.forEach((m: {
      ...
      return msgComp.push(<Message id={m.message_id} subscribeToScroll={(c: Function) => subscribeToScroll(c)} isRead={m.read} content={m.content} pub_key={props.pub_key} key={m.message_id} fromMe={m.fromMe} date={m.date} />)
    });
    return msgComp
  ...
  return <>
    <div className={'chat_dialog'}>
      ...
      <div className={'chat_dialog__messages'} onScroll={(e) => scrollCallback(e)} ref={messagesDiv}>{mes}</div>
